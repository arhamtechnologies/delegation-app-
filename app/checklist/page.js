...
export default function Checklist() {
  ...
  const filteredItems = useMemo(() => items.filter((item) => {
    ...
    const matchesSearch = !query || [item.task, item.employee?.name, item.employee?.email, formatEmployeeId(item.employee_id)].filter(Boolean).join(' ').toLowerCase().includes(query);
