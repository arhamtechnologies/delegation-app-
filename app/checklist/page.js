...
import { canManageChecklists, checklistFrequencies, checklistWeekdays, formatChecklistDays, formatEmployeeId, getBusinessDate, getChecklistStatus, triggerChecklistGeneration } from '../../lib/checklist-data';
import { supabaseBrowser } from '../../lib/supabase-browser';

const emptyForm = { employee_id: '', task: '', frequency: 'daily', weekday: '1', start_date: '', day_of_month: '1', active: true };
...
export default function Checklist() {
  ...
  async function load() {
    ...
    const itemQuery = supabase.from('checklist_items').select('id,template_id,employee_id,task,due_date,status,completed_at,completed_by,created_at,employee:employees!checklist_items_employee_id_fkey(id,name,email)').order('due_date', { ascending: false }).limit(500);
    const [itemResponse, templateResponse, employeeResponse] = await Promise.all([
      itemQuery,
      manager ? supabase.from('checklist_templates').select('id,employee_id,task,frequency,weekday,day_of_month,start_date,active,created_at,updated_at,employee:employees!checklist_templates_employee_id_fkey(id,name,email)').order('active', { ascending: false }).order('created_at', { ascending: false }) : Promise.resolve({ data: [], error: null }),
  ...
  function openEdit(template) {
    setEditing(template);
    setForm({ employee_id: template.employee_id, task: template.task, frequency: template.frequency, weekday: String(template.weekday ?? 1), start_date: template.start_date || '', day_of_month: String(template.day_of_month ?? 1), active: template.active !== false });
    setError('');
    setModalOpen(true);
  }
  ...
  async function save(event) {
    ...
    setSaving(true);
    setError('');
    if (!form.employee_id || !form.task.trim()) { setError('Choose an employee and add a task.'); setSaving(false); return; }
    if (form.frequency === 'weekly' && form.weekday === '') { setError('Choose a day of the week.'); setSaving(false); return; }
    if (form.frequency === 'every_15_days' && !form.start_date) { setError('Choose a start date.'); setSaving(false); return; }
    ...
    const record = {
      ...
      start_date: form.frequency === 'every_15_days' ? form.start_date : null,
      active: form.active,
    };
    const supabase = supabaseBrowser();
  ...
  async function completeItem(item) {
    ...
    setError('');
    const { error: updateError } = await supabaseBrowser().from('checklist_items').update({ status: 'completed', completed_at: new Date().toISOString(), completed_by: data.userId }).eq('id', item.id);
    if (updateError) setError(updateError.message);
  ...
  return <AppShell title="Checklist" eyebrow="Workspace / Checklist" description="Manage recurring daily, weekly, and monthly work for employees." actions={titleAction}>
    ...
    {canManage && <Modal open={modalOpen} title={editing ? 'Edit checklist' : 'Add checklist'} description="Create a recurring rule for one employee. Generated history stays unchanged when you edit the rule." onClose={closeModal} wide><form className="modal-form" onSubmit={save}><div className="form-grid form-grid-two"><div className="field field-wide"><label htmlFor="checklist-employee">Employee<span>*</span></label><select id="checklist-employee" className="input" required value={form.employee_id} onChange={(event) => updateForm('employee_id', event.target.value)}><option value="">Choose an employee</option>{employees.filter((employee) => employee.active).map((employee) => <option value={employee.id} key={employee.id}>{formatEmployeeId(employee.id)} — {employee.name}</option>)}</select></div><div className="field field-wide"><label htmlFor="checklist-task">Task<span>*</span></label><input id="checklist-task" className="input" required maxLength="240" placeholder="e.g. Check daily sales report" value={form.task} onChange={(event) => updateForm('task', event.target.value)} /></div><div className="field"><label htmlFor="checklist-frequency">Frequency<span>*</span></label><select id="checklist-frequency" className="input" required value={form.frequency} onChange={(event) => updateForm('frequency', event.target.value)}>{checklistFrequencies.map((frequency) => <option value={frequency.value} key={frequency.value}>{frequency.label}</option>)}</select></div>{form.frequency === 'weekly' && <div className="field"><label htmlFor="checklist-weekday">Day of week<span>*</span></label><select id="checklist-weekday" className="input" required value={form.weekday} onChange={(event) => updateForm('weekday', event.target.value)}>{checklistWeekdays.map((weekday, index) => <option value={index} key={weekday}>{weekday}</option>)}</select></div>}{form.frequency === 'every_15_days' && <div className="field"><label htmlFor="checklist-start-date">Start date<span>*</span></label><input id="checklist-start-date" className="input" type="date" required value={form.start_date} onChange={(event) => updateForm('start_date', event.target.value)} /></div>}{form.frequency === 'monthly' && <div className="field"><label htmlFor="checklist-day-of-month">Day of month<span>*</span></label><select id="checklist-day-of-month" className="input" required value={form.day_of_month} onChange={(event) => updateForm('day_of_month', event.target.value)}>{Array.from({ length: 31 }, (_, index) => index + 1).map((day) => <option value={day} key={day}>{day}</option>)}</select><small className="field-help">If a month is shorter, the item is generated on that month&apos;s last day.</small></div>}<label className="checkbox-field field-wide"><input type="checkbox" checked={form.active} onChange={(event) => updateForm('active', event.target.checked)} /><span><strong>Active</strong><small>Deactivate recurring work without deleting its generated history.</small></span></label></div>{error && <div className="form-error"><Icon name="warning" size={16} />{error}</div>}<div className="modal-actions"><button className="button button-ghost" type="button" onClick={closeModal}>Cancel</button><button className="button button-primary" type="submit" disabled={saving}>{saving ? 'Saving...' : editing ? 'Save changes' : 'Add checklist'}{!saving && <Icon name="arrowUpRight" size={16} />}</button></div></form></Modal>}
  </AppShell>;
}
