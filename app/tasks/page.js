...
import { canCompleteChecklist, formatChecklistDueAt, getChecklistItems, getChecklistStatus, setChecklistCompletion } from '../../lib/checklist-data';
  ...
  return <AppShell title={taskData ? 'Tasks' : 'Loading tasks'} eyebrow="Workspace / Tasks" description="Create, prioritize, and keep every assignment moving." actions={canCreate ? <button className="button button-primary" type="button" onClick={openCreateModal}><Icon name="plus" size={17} />Create task</button> : null}>
    ...
    <section className="panel task-panel">
      ...
      <div className="filter-bar">
        ...
        <label className="filter-control"><span>Status</span><select value={status} onChange={(event) => setStatus(event.target.value)} disabled={!taskData}><option value="all">All status</option><option value="pending">Pending</option><option value="overdue">Overdue</option><option value="completed">Completed</option></select></label>
