...
export default function Tasks() {
  ...
  useEffect(() => {
    if (canCreate && new URLSearchParams(window.location.search).get('create') === '1') setModalOpen(true);
  }, [canCreate]);
  ...
  return <AppShell title="Tasks" eyebrow="Workspace / Tasks" description="Create, prioritize, and keep every assignment moving." actions={canCreate ? <button className="button button-primary" type="button" onClick={() => setModalOpen(true)}><Icon name="plus" size={17} />Create task</button> : null}>
