...
export default function MIS() {
  ...
  const load = useCallback(async (overrides = {}) => {
    ...
    const [{ data: taskRows = [] } = {}, { data: eligibleEmployees = [] } = {}] = await Promise.all([
      getTasks({
        limit: 500,
        select: 'assignee_id,status,eta,completed_at',
        status: selectedStatus === 'all' ? undefined : selectedStatus,
        etaFrom: selectedFrom ? new Date(`${selectedFrom}T00:00:00`).toISOString() : undefined,
        etaTo: selectedTo ? new Date(`${selectedTo}T23:59:59.999`).toISOString() : undefined,
      }),
      getTaskAssignees(),
    ]);
