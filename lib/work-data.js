...
export function getTodaysWorkItems(workItems, now = new Date(), timeZone = defaultChecklistTimeZone) {
  ...
  return (workItems || [])
    ...
    .sort((left, right) => {
      const leftStatus = getWorkItemStatus(left, now);
      const rightStatus = getWorkItemStatus(right, now);
      const statusDifference = Number(leftStatus === 'completed') - Number(rightStatus === 'completed');
