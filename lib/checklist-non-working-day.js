...
function restoredChecklistStatus(item, date, now = new Date()) {
  if (item.due_at) {
    const dueAt = new Date(item.due_at);
    if (!Number.isNaN(dueAt.getTime())) return now.getTime() > dueAt.getTime() ? 'overdue' : 'pending';
  }
  return getChecklistDateInTimeZone(now) > date ? 'overdue' : 'pending';
}

export async function restoreChecklistItemsAfterEmployeeLeaveRemoval(admin, employeeId, date) {
  if (isSunday(date)) return 0;

  const [holidayData, leaveResponse] = await Promise.all([
    loadHolidayRows(admin, date),
    admin.from('employee_non_working_days').select('id').eq('employee_id', employeeId).eq('non_working_date', date).eq('reason', 'employee_leave'),
  ]);
  if (leaveResponse.error) throw leaveResponse.error;
  if (holidayData.activeRows.length || (leaveResponse.data || []).length) return 0;
  ...
  const now = new Date();
  const itemsByStatus = new Map([['pending', []], ['overdue', []]]);
  items.forEach((item) => itemsByStatus.get(restoredChecklistStatus(item, date, now)).push(item.id));
  let restoredCount = 0;
  for (const [status, ids] of itemsByStatus) {
    if (!ids.length) continue;
    const { data: restoredItems, error } = await admin
      .from('checklist_items')
      .update({ status })
      .in('id', ids)
      .eq('status', 'deactivated')
    ...
    restoredCount += restoredItems?.length || 0;
  }
...
async function loadAllOpenChecklistItems(admin) {
  const items = [];
  for (let from = 0; ; from += pageSize) {
    const response = await admin
      .from('checklist_items')
      .select('id,employee_id,due_date,due_at,status')
      .in('status', activeChecklistStatuses)
...
async function loadNonWorkingRecordsForDates(admin, dates) {
  const holidays = [];
  const leaves = [];
  for (let from = 0; from < dates.length; from += updateBatchSize) {
    const dateBatch = dates.slice(from, from + updateBatchSize);
    const [holidayResponse, leaveResponse] = await Promise.all([
      admin
        .from('national_holidays')
        .select('holiday_date')
        .in('holiday_date', dateBatch)
        .eq('country', 'India')
        .eq('is_active', true),
      admin
        .from('employee_non_working_days')
        .select('employee_id,non_working_date,employee:employees!employee_non_working_days_employee_id_fkey(active)')
        .in('non_working_date', dateBatch)
        .eq('reason', 'employee_leave'),
    ]);
    if (holidayResponse.error) throw holidayResponse.error;
    if (leaveResponse.error) throw leaveResponse.error;
    holidays.push(...(holidayResponse.data || []));
    leaves.push(...(leaveResponse.data || []).filter((leave) => leave.employee?.active !== false));
  }
  return { holidays, leaves };
}

async function updateChecklistItemStatuses(admin, ids, status, expectedStatuses) {
  let updatedCount = 0;
  for (let from = 0; from < ids.length; from += updateBatchSize) {
    const response = await admin
      .from('checklist_items')
      .update({ status })
      .in('id', ids.slice(from, from + updateBatchSize))
...
// Generation is the final safety net for non-working-day records configured
// before their occurrences exist. It also repairs any open occurrence missed
// by an earlier run before the occurrence can be classified as overdue.
export async function reconcileOpenChecklistItemsForNonWorkingDays(admin, now = new Date()) {
  const items = await loadAllOpenChecklistItems(admin);
  const dates = [...new Set(items.map((item) => item.due_date).filter(Boolean))];
  if (!dates.length) return { scanned: 0, deactivated: 0, markedOverdue: 0 };

  const { holidays, leaves } = await loadNonWorkingRecordsForDates(admin, dates);
  const holidayDates = new Set(holidays.map((holiday) => holiday.holiday_date));
  const leaveKeys = new Set(leaves.map((leave) => `${leave.employee_id}:${leave.non_working_date}`));
  const nonWorkingIds = [];
  const overdueIds = [];
  const nowTime = now.getTime();

  items.forEach((item) => {
    const isNonWorking = isSunday(item.due_date)
      || holidayDates.has(item.due_date)
      || leaveKeys.has(`${item.employee_id}:${item.due_date}`);
    if (isNonWorking) {
      nonWorkingIds.push(item.id);
      return;
    }
    if (item.status !== 'pending' || !item.due_at) return;
    const dueAt = new Date(item.due_at);
    if (!Number.isNaN(dueAt.getTime()) && dueAt.getTime() < nowTime) overdueIds.push(item.id);
  });

  const deactivated = await updateChecklistItemStatuses(admin, nonWorkingIds, 'deactivated', activeChecklistStatuses);
  const markedOverdue = await updateChecklistItemStatuses(admin, overdueIds, 'overdue', ['pending']);
  return { scanned: items.length, deactivated, markedOverdue };
}
