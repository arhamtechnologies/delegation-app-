...
export async function parseChecklistWorkbook(buffer, { filename = '', employees = [], existingTemplates = [], startDate } = {}) {
  ...
  const sheetName = 'First worksheet';
  const headerIndex = matrix.findIndex((row) => checklistImportHeaders.every((header) => row.some((value) => normalizeHeader(value) === normalizeHeader(header))));
