...
export async function getChecklistItems({ limit = 200, dueDate, dueDateFrom, dueDateTo, includeDeactivated = false, employeeId, status, nowIso = new Date().toISOString(), select = 'id,template_id,employee_id,task,due_date,due_at,status,completed_at,completed_by,created_at,employee:employees!checklist_items_employee_id_fkey(id,name,email),template:checklist_templates!checklist_items_template_id_fkey(frequency,weekday,day_of_month,monthly_days,start_date,due_time)' } = {}) {
  ...
  if (status === 'overdue') query = query.neq('status', 'completed').is('completed_at', null).lt('due_at', nowIso);
  if (status === 'pending') query = query.neq('status', 'completed').is('completed_at', null).gte('due_at', nowIso);
