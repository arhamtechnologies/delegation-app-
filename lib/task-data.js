...
const taskListSelect = 'id,title,description,priority,status,assignee_id,eta,completed_at,category,assignee:employees!tasks_assignee_id_fkey(id,name,email,role)';
const taskDetailSelect = 'id,title,description,priority,status,assignee_id,eta,start_date,category,instructions,proof_required,completion_notes,attachments,created_at,updated_at,completed_at,assignee:employees!tasks_assignee_id_fkey(id,name,email,role)';
...
export async function getTasks({ limit = 100 } = {}) {
  ...
  const response = await supabase.from('tasks').select(taskListSelect).order('updated_at', { ascending: false }).limit(limit);
  if (response.error && response.error.message?.includes('updated_at')) {
    return supabase.from('tasks').select(taskListSelect).order('created_at', { ascending: false }).limit(limit);
  }
  return response;
}
