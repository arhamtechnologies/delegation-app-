...
export function getTaskStatus(task, now = new Date()) {
  ...
  const dueDate = localDateNumber(task?.eta);
  const today = localDateNumber(now);
  return dueDate !== null && today !== null && dueDate < today ? 'overdue' : 'pending';
}

export function isTaskCompletedOnTime(task) {
  if (getTaskStatus(task) !== 'completed' || !task?.completed_at || !task?.eta) return false;
  const completedDate = localDateNumber(task.completed_at);
  const dueDate = localDateNumber(task.eta);
  return completedDate !== null && dueDate !== null && completedDate <= dueDate;
}
...
export async function getDashboardData(manager) {
  ...
  const overdueCount = activeTaskQuery(supabase.from('tasks').select('id', { count: 'exact', head: true }).lt('eta', todayStart.toISOString()));
  const priorityQueries = [
    activeTaskQuery(supabase.from('tasks').select(taskSummarySelect).lt('eta', todayStart.toISOString()).order('eta', { ascending: true }).limit(8)),
    activeTaskQuery(supabase.from('tasks').select(taskSummarySelect).gte('eta', todayStart.toISOString()).lte('eta', soonIso).order('eta', { ascending: true }).limit(8)),
  ];
  const metricQueries = manager
    ? [
      ['total', supabase.from('tasks').select('id', { count: 'exact', head: true })],
      ['overdue', overdueCount],
      ['dueSoon', activeTaskQuery(supabase.from('tasks').select('id', { count: 'exact', head: true }).gte('eta', todayStart.toISOString()).lte('eta', soonIso))],
    ]
    : [
