...
const taskSummarySelect = 'id,title,priority,status,eta,category,assignee_id,completed_at,assignee:employees!tasks_assignee_id_fkey(name)';
...
export async function getTaskEmployees() {
  return supabaseBrowser().from('employees').select('id,name,active,role').eq('active', true).order('name');
}
