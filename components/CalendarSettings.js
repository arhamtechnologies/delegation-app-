...
export default function CalendarSettings() {
  ...
  return <>
    ...
    <section className="panel checklist-panel non-working-day-settings"><SectionHeader eyebrow="Calendar" title="Non-working day" description="Sunday is automatic. Add employee leave dates here." action={<button className="button button-ghost button-small" type="button" onClick={openEmployeeLeaveModal}><Icon name="plus" size={15} />Employee Leave</button>} /><div className="configured-non-working-days">{employeeLeave.flatMap((group) => group.dates.map((dateValue) => <div className="configured-non-working-day-row" key={`${group.employeeId}-${dateValue}`}><div><div className="configured-non-working-day-heading"><strong>{group.employee?.name || formatEmployeeId(group.employeeId)}</strong><strong>{formatSelectedDate(dateValue)}</strong></div><span>Employee Leave</span></div><div className="holiday-actions"><button className="button button-ghost button-small" type="button" onClick={() => openEmployeeLeaveConfig(group)}>Edit</button><button className="button button-ghost button-small" type="button" onClick={() => removeConfiguredLeaveDate(group, dateValue)}>Remove</button></div></div>))}</div>{!employeeLeave.length && <EmptyState compact icon="calendar" title="No employee leave dates configured" description="National holidays are listed above. Sundays are detected automatically." />}</section>
